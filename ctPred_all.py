...
# All 23 skin cell types to train
cell_types = [
    "Arterial_EC", "Basal_KC", "Bulb", "Bulge",
...
models_dir    = '/home/tooba/Skin_Cell_Model/models/'
...
def plot_prediction(model, test_features, test_labels, png_path):
    model.eval()
...
# Loop over all 23 skin cell types
for ct in cell_types:
    print(f"\n{'='*60}")
    print(f"Training cell type: {ct}")
    print(f"{'='*60}")

    gen_data_p = os.path.join(processed_dir, f'processed_IbG_pseudobulk_{ct}.csv')

    if not os.path.exists(gen_data_p):
        print(f"  WARNING: File not found, skipping: {gen_data_p}")
        continue
    ...
    saved_path = os.path.join(models_dir, f'scPred_{ct}.pt')
    png_path   = os.path.join(models_dir, f'scPred_{ct}.png')

    scPred_test = scPred().to(device)
    scPred_training(scPred_test, train_data_iter, val_epi, val_exp, epochs=100, model_path=saved_path)

    plot_prediction(scPred_test, test_epi, test_exp, png_path)

    scPred_reload = load_model(saved_path, scPred)
    print(f"  Reloaded and verified: {saved_path}")

print(f"\n{'='*60}")
print("ALL 23 CELL TYPES COMPLETE")
